<?php
namespace  zmxy\src;
use zmxy\src\util\RSAUtil;
use zmxy\src\util\WebUtil;
use zmxy\src\log\LtLogger;
...
class ZmopClient {
	...
	function __construct($gatewayUrl = '', $appId = '', $charset = 'UTF-8', $privateKeyFilePath, $zhiMaPublicKeyFilePath,$logDir='/tmp'){
	...
	public function execute($request) {
		...
		if (isset ($bizRespObject->error_code)) {
			$logger = new LtLogger;
			$logger->conf["log_file"] = rtrim($this->logDir, '\\/') . '/' . "logs/zmop_biz_err_" . $this->appId . "_" . date("Y-m-d") . ".log";
			$logger->log(array(
					date("Y-m-d H:i:s"),
					$resp
				));
